...
from app.services.master_data import delete_customer, list_customers, save_customer
...
class CustomerTab(QWidget):
    def __init__(self, parent=None):
        ...
        self.table.setHorizontalHeaderLabels(["Name", "Contact"])
    ...
    def _add(self):
        ...
        if dialog.exec():
            name, contact = dialog.values()
            with new_session() as session:
                save_customer(session, None, name, contact)
                session.commit()
            self.refresh()
    ...
    def _edit(self):
        ...
        if dialog.exec():
            name, contact = dialog.values()
            with new_session() as session:
                save_customer(session, row_id, name, contact)
                session.commit()
            self.refresh()
    ...
    def _delete(self):
        ...
        if row_id is None:
            ...
            return
        if QMessageBox.question(self, "Confirm delete", "Delete this customer?") == QMessageBox.Yes:
            with new_session() as session:
                try:
                    delete_customer(session, row_id)
                    session.commit()
                except IntegrityError:
                    session.rollback()
                    QMessageBox.warning(self, "Cannot delete", "This customer is used by sale records.")
            self.refresh()
